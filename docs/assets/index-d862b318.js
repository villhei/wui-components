...
`,x=["text","default","success","primary","secondary","warning","error","background"];class i extends HTMLElement{constructor(t){super();a(this,"root");const o=this.attachShadow({mode:"open"});o.appendChild(t.content.cloneNode(!0)),this.root=o}attributeChangedCallback(){this.update()}getBoolean(t){return this.getAttribute(t)===""}forwardAttribute(t,o){const n=this.getAttribute(t);n===null?o.removeAttribute(t):o.setAttribute(t,n)}update(){}}const d=document.createElement("template");d.innerHTML=y;class w extends i{constructor(){super(d);a(this,"anchorNode");this.anchorNode=this.root.querySelector("a")}static get observedAttributes(){return["href"]}update(){this.forwardAttribute("href",this.anchorNode)}}const k=`<style>
  button {
    outline: none;
    border: none;
...
<div><slot></slot></div>
`,h=document.createElement("template");h.innerHTML=N;class C extends i{constructor(){super(h);a(this,"rootNode");this.rootNode=this.root.querySelector("div"),this.update()}static get observedAttributes(){return["class","raised","inline","direction","wrap","position"]}get inline(){return this.getBoolean("inline")}get raised(){return this.getBoolean("raised")}get classes(){var t;return((t=this.getAttribute("class"))==null?void 0:t.split(","))||[]}get direction(){return this.getAttribute("direction")||"row"}get wrap(){return this.getAttribute("wrap")||"nowrap"}get position(){return this.getAttribute("position")}update(){this.rootNode.style.setProperty("--flex-direction",this.direction),this.rootNode.style.setProperty("--flex-wrap",this.wrap),this.rootNode.style.setProperty("--position",this.position),this.raised?this.rootNode.classList.add("raised"):this.rootNode.classList.remove("raised"),this.inline?(this.rootNode.classList.add("inline-flex"),this.rootNode.classList.remove("flex")):(this.rootNode.classList.remove("inline-flex"),this.rootNode.classList.add("flex")),this.classes.forEach(t=>this.rootNode.classList.add(t))}}const E=`<style>
  .aligner {
...
`,S=(r,e)=>{const t=Math.abs(e-r);return Array.apply(null,Array(t)).map(function(o,n){return r+n})},u={h1:3.125,h2:2.375,h3:1.75,h4:1.31,h5:1,p:1,tooltip:.875,small:.75},H=S(1,6).map(r=>{const e=`h${r}`,t=document.createElement("template"),o=u[e],n=Math.max(1,o*.4).toPrecision(3);return t.innerHTML=`
    <style>
      ${e} {
          --border-width: 0;
          --bottom-padding: 0;
      ...
      <${e}><slot>Foo</slot></${e}>
    `,[e,t]}),M=Object.fromEntries(H),g=document.createElement("template");g.innerHTML=$;const q={...M,p:g};class l extends i{constructor(t){super(q[t]);a(this,"tag");this.tag=this.root.querySelector(t)}static get observedAttributes(){return["underline","color"]}get underline(){return this.getAttribute("underline")===""}get color(){return this.getAttribute("color")}update(){if(this.underline?(this.tag.style.setProperty("--border-width","var(--border-width-underline)"),this.tag.style.setProperty("--padding-bottom","var(--border-width-underline)")):this.tag.style.setProperty("--border-width","0"),this.color){const t=this.color,o=x.includes(t)?`var(--${t}-color)`:t;this.tag.style.setProperty("--font-color",o)}}}class T extends l{constructor(){super("h1")}}class I extends l{constructor(){super("h2")}}class B extends l{constructor(){super("h3")}}class O extends l{constructor(){super("h4")}}class F extends l{constructor(){super("h5"),this.tag.style.setProperty("text-transform","uppercase")}}class j extends l{constructor(){super("p");a(this,"pNode");this.pNode=this.root.querySelector("p")}get small(){return this.getBoolean("small")}static get observedAttributes(){return["small"]}update(){this.small?this.pNode.style.setProperty("--font-size",`${u.small}rem`):this.pNode.style.setProperty("--font-size",`${u.paragraph}rem`)}}customElements.define("wui-a",w);customElements.define("wui-button",A);customElements.define("wui-flex",C);customElements.define("wui-input",z);customElements.define("wui-icon",P);customElements.define("wui-h1",T);customElements.define("wui-h2",I);customElements.define("wui-h3",B);customElements.define("wui-h4",O);customElements.define("wui-h5",F);customElements.define("wui-p",j);
